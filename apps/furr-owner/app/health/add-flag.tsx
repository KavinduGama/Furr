...
// ─────────────────────────────────────────────────────────────
//  Add Health Flag (Allergies/Conditions) screen (HLT-002)
// ─────────────────────────────────────────────────────────────
...
export default function AddFlagScreen() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await createFlag(firebaseUser.uid, selectedPet.id, {
        type,
        title: title.trim(),
        status,
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
      <View style={styles.topBar}>
        ...
        <Text style={styles.heading}>Add flag</Text>
        <View style={{ width: 60 }} />
      </View>
      ...
      <View style={styles.section}>
        ...
        <View style={styles.rowGrid}>
          ...
          >
            <Ionicons name="warning" size={18} color={type === 'allergy' ? '#E65100' : colors.muted} />
          ...
          >
            <Ionicons name="medkit" size={18} color={type === 'condition' ? colors.brand : colors.muted} />
      ...
      <Button label="Save flag" loading={saving} onPress={handleSave} disabled={!title.trim()} />
...
const styles = StyleSheet.create({
  ...
  typeCard: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: colors.surface, borderRadius: radius.md, paddingVertical: 12, borderWidth: 1.5, borderColor: colors.line },
  typeLabel: { color: colors.muted, fontSize: 14, fontWeight: '900' },
  statusCard: { flex: 1, alignItems: 'center', backgroundColor: colors.surface, borderRadius: radius.md, paddingVertical: 12, borderWidth: 1.5, borderColor: colors.line },
  statusCardActive: { borderColor: colors.brand, backgroundColor: colors.mist },
  statusLabel: { color: colors.muted, fontSize: 13, fontWeight: '800' },
  ...
  disclaimer: { flexDirection: 'row', gap: 8, backgroundColor: colors.mist, padding: 12, borderRadius: radius.md, alignItems: 'center' },
  disclaimerText: { color: colors.brand, fontSize: 11, lineHeight: 15, flex: 1, fontWeight: '700' },
});
